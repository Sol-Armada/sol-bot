CREATE OR REPLACE FUNCTION notify_domain_change(channel_name TEXT, pk_columns TEXT[])
RETURNS TRIGGER AS $$
DECLARE
...
FOR EACH ROW
EXECUTE FUNCTION notify_domain_change('solbot_docs', ARRAY['id']);
...
FOR EACH ROW
EXECUTE FUNCTION notify_domain_change('solbot_rsi', ARRAY['handle']);

CREATE TRIGGER trg_notify_activity_logs_change
AFTER INSERT OR UPDATE OR DELETE ON activity_logs
FOR EACH ROW
EXECUTE FUNCTION notify_domain_change('solbot_logs', ARRAY['id']);
